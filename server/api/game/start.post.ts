...
export default defineEventHandler(async (event) => {
  ...
  if (!cached.length) {
    const [freshSubject, relatedIds] = await Promise.all([getSubjectWithCharacters(subjectId), getRelatedSubjectIds(subjectId)]);
  ...
  return {
    sessionId,
    message: '新游戏已开始。你可以继续提问、要提示，或直接搜索并提交答案。',
    initialHint,
    remainingHints: HINT_LIMIT - 1,
